...
# Ignores alternative, random, and unplaced chromosomes
...
# Drop alternate chromosomes
keepSeqs <- grep(
  "_", seqnames(genome), fixed = TRUE, invert = TRUE, value = TRUE)
genome@user_seqnames <- setNames(keepSeqs, keepSeqs)
genome@seqinfo <- genome@seqinfo[keepSeqs]
